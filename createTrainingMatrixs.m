function createTrainingMatrixs(rootFolder)
    ...
    referenceFileName = [trainingDataFolder 'REFERENCE.csv'];
    ...
    matFiles = dir([trainingDataFolder '*.mat']);
    ...
    for windowSize = [64 128 256]  %total number of matrixs
        ...
        for matrixCount = 1:length(matFiles)  %window sizes
            ...
            load([trainingDataFolder matFiles(matrixCount).name]);
            ...
            %add class/label
            for refCount = lastRefCount:size(referenceFileData,1)
                ref = strsplit(referenceFileData(refCount,:),',');
        ...
        if(windowSize == 64)
             save(folder64,'mtrx');
             disp('training matrix for 64 window is saved');
        end
        if(windowSize == 128)
             save(folder128,'mtrx');
             disp('training matrix for 128 window is saved');
        end
        if(windowSize == 256)
%              save(folder256,'mtrx');
             disp('training matrix for 256 window is saved');
             fid=fopen('output.txt','a+');
             fprintf(fid, 'Training Data Statistics:\n');
             fprintf(fid, '%s\n',['N = ' num2str(typ(1,1))]);
             fprintf(fid, '%s\n',['A = ' num2str(typ(2,1))]);
             fprintf(fid, '%s\n',['O = ' num2str(typ(3,1))]);
             fprintf(fid, '%s\n',['~ = ' num2str(typ(4,1))]);
